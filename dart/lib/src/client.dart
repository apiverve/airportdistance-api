...
class AirportdistanceValidationException implements Exception {
  final List<String> errors;

  AirportdistanceValidationException(this.errors);

  @override
  String toString() => 'AirportdistanceValidationException: ${errors.join("; ")}';
}
...
/// Airport Distance API client.
///
/// For more information, visit: https://airportdistance.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [iata1] (required) - The IATA code of the first airport (e.g. JFK)
/// * [iata2] (required) - The IATA code of the second airport (e.g. LAX)
class AirportdistanceClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'iata1': ValidationRule(type: 'string', required: true),
    'iata2': ValidationRule(type: 'string', required: true),
  };

  AirportdistanceClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/airportdistance',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [AirportdistanceValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw AirportdistanceValidationException(errors);
    }
  }

  /// Execute a request to the Airport Distance API.
  ///
  /// Parameters are validated before sending the request.
  Future<AirportdistanceResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw AirportdistanceException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return AirportdistanceResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw AirportdistanceException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw AirportdistanceException('Resource not found');
      } else {
        throw AirportdistanceException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is AirportdistanceException) rethrow;
      throw AirportdistanceException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Airport Distance API client.
class AirportdistanceException implements Exception {
  final String message;

  AirportdistanceException(this.message);

  @override
  String toString() => 'AirportdistanceException: $message';
}
